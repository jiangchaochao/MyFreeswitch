...
/* Main monitor thread (1 per distinct conference room)  主监视线程，每个会议室一个*/
void *SWITCH_THREAD_FUNC conference_thread_run(switch_thread_t *thread, void *obj)
{
    ...
    /*ADD_by jiangc@2018/8/4 增加计时，用来降低静音包发送频率*/
    long mytime = 0;
	    mytime = get_time();
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "会议监视线程启动时间为：%ld\n", mytime);
	if (!(divisor = conference->rate / 8000)) {
		divisor = 1;
	}
	/*申请大小SWITCH_RECOMMENDED_BUFFER_SIZE的空间 */
	...
	while (conference_globals.running && !conference_utils_test_flag(conference, CFLAG_DESTRUCT)) {
		...
		/* Sync the conference to a single timing source 将会议同步到单个定时源*/
		...
		for (imember = conference->members; imember; imember = imember->next) {
			...
			if (switch_buffer_inuse(imember->audio_buffer) >= bytes
				...
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, " 保存一帧数据\n");
			...
			if (conference->async_fnode->leadin) {
				...
				if (file_sample_len <= 0) {
					...
					if (has_file_data) {
						...
						/*Mod by jiangc@2018/8/4 这里修改为只有一个人时不播放等待铃声*/
		...
		if (ready || has_file_data) {
			/* Use more bits in the main_frame to preserve the exact sum of the audio samples.  在main_frame中使用更多位来保留音频样本的精确总和*/
			int main_frame[SWITCH_RECOMMENDED_BUFFER_SIZE] = { 0 };
			int16_t write_frame[SWITCH_RECOMMENDED_BUFFER_SIZE] = { 0 };


			/* Init the main frame with file data if there is any. 如果有的话，用文件数据初始化主框架*/
			...
			/* Copy audio from every member known to be producing audio into the main frame. 将已知要生成音频的每个成员的音频复制到主框架中*/
			for (omember = conference->members; omember; omember = omember->next) {
			...
			/* Create write frame once per member who is not deaf for each sample in the main frame
			   ...
			   每个对主框架中的每个样本都没有聋的成员创建一次写入帧检查是否涉及我们的音频，如果是，则从样本中减去它，
            这样我们就不会听到自己。因为主帧是32位int，我们做了 不丢失任何细节，现在我们必须转换为16位，如果需要我们可
            以在最小和最大范围切断它并将帧写入输出缓冲区。
			...
			*/
			for (omember = conference->members; omember; omember = omember->next) {
				...
				if (!conference_utils_member_test_flag(omember, MFLAG_CAN_HEAR)) {
					...
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "推入的静音包##############################\n");
		...
		} else { /* There is no source audio.  Push silence into all of the buffers jiangc@2018/8/4  没有源音频。 将静音推入所有缓冲区 前推入静音包 ==============##############!!!!!!!!!!!!!!!!$$$$$$$$$$$$$这里是重点 */
			int16_t write_frame[SWITCH_RECOMMENDED_BUFFER_SIZE] = { 0 };

			if (((get_time() - mytime > 0) && (get_time() - mytime > 3000)) || ((get_time() - mytime < 0) && (get_time() - mytime > -3000)))
            {
                mytime = get_time();      
			if (conference->comfort_noise_level) {
				/*产生静电噪音*/
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, " 产生静电噪音;\n");
...
/* Application interface function that is called from the dialplan to join the channel to a conference  会议入口*/
SWITCH_STANDARD_APP(conference_function)
{
